...
def _all(input, ):
    ...
    def sum_x_1(sum_x_s1_y, sum_x_s1_x, input, sum_x, ):
        ...
        return (sum_x[sum_x_s1_y, sum_x_s1_x] + _sum(
            ...
            expr = ((hcl.cast(dtype = hcl.UInt(bits = 32), expr = input[(sum_x_s1_win2__y + sum_x_s1_y), (sum_x_s1_win2__x + sum_x_s1_x)]) * hcl.cast(dtype = hcl.UInt(bits = 32), expr = hcl.cast(dtype = hcl.UInt(bits = 8), expr = (hcl.call_pure_intrin("float", "pow", 2.718280, (hcl.cast(dtype = hcl.Float(bits = 32), expr = ((4 - sum_x_s1_win2__x) * (sum_x_s1_win2__x + -4))) * float(0.222222))) * float(67.972794))))) * hcl.cast(dtype = hcl.UInt(bits = 32), expr = hcl.cast(dtype = hcl.UInt(bits = 8), expr = (hcl.call_pure_intrin("float", "pow", 2.718280, (hcl.cast(dtype = hcl.Float(bits = 32), expr = ((4 - sum_x_s1_win2__y) * (sum_x_s1_win2__y + -4))) * float(0.222222))) * float(67.972794)))))
        ))
    ...
    def blur_x_0(blur_x_s0_y, blur_x_s0_x, input, sum_x, ):
        return hcl.cast(dtype = hcl.UInt(bits = 8), expr = (sum_x[blur_x_s0_y, blur_x_s0_x]/65536))
    ...
    return output_0
input = hcl.placeholder((12, 12, ), name = "input", dtype = hcl.UInt(bits = 8))
