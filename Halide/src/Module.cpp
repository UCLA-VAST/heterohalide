...
// Internal::CodeGen_HeteroCL Module::compile_heterocl(const Outputs &output_files_arg, const std::vector<Function> &output_funcs, const std::vector<int> &output_shape) const {
//     Outputs output_files = output_files_arg;
//     user_assert(!output_files.heterocl_name.empty()) << "must be compiled to HeteroCL";
//     debug(1) << "Module.compile(): heterocl_name " << output_files.heterocl_name << "\n";
//     debug(1) << "output shape: \n";
//     for (int x : output_shape) {
//         debug(1) << x << "\n";
//     }
//     std::ofstream file;
//     file.open(output_files.heterocl_name, std::ios_base::app);
//     Internal::CodeGen_HeteroCL cg(file,
//                                   output_funcs,
//                                   output_shape,
//                                   target());
//     cg.compile(*this);
...
void Module::compile(const Outputs &output_files_arg, const std::vector<Function> &output_funcs, const std::vector<int> &output_shape) const { // the parameter output_shape is for compiling heterocl code only
    Outputs output_files = output_files_arg;
    ...
    if (!output_files.heterocl_name.empty()) {
        debug(1) << "Module.compile(): heterocl_name " << output_files.heterocl_name << "\n";
        debug(1) << "output shape: \n";
        for (int x : output_shape) {
            debug(1) << x << "\n";
        }
        std::ofstream file(output_files.heterocl_name);
        // file.open(output_files.heterocl_name, std::ios_base::app);
        Internal::CodeGen_HeteroCL cg(file,
                                      output_funcs,
                                      output_shape,
