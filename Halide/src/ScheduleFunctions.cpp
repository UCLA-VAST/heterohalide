...
Stmt build_loop_nest(
    ...
    debug(1) << "vector splits size: " << splits.size() << "\n"; // 1
    for (auto iter = splits.begin(); iter != splits.end(); ++iter) {
        debug(1) << iter->old_var << ", " << iter->outer << ", " << iter->inner << "\n"; // x, x.x_outer, x.x_inner
        debug(1) << "split type: " << iter->split_type << "\n"; // 0:SplitVar
        debug(1) << "factor: " << iter->factor << "\n"; // class Expr, but in this sample can be printed directly: 2
    }
    ...
    debug(1) << "build loop nest: splits\n" << stmt << "\n";
    // All containing lets and fors. Outermost first.
    vector<Container> nest;
    debug(1) << "Print vector<Container> nest:\n";
    ...
    debug(1) << "build loop nest: container\n" << stmt << "\n";
    ...
    debug(1) << "build loop nest: rewrap the stmt in the containing lets and fors\n" << stmt << "\n";
...
protected:
    ...
    Stmt visit(const For *for_loop) override {
        debug(1) << "Injecting " << funcs << " entering for-loop over " << for_loop->name << "\n";
        ...
        if (funcs[0].has_extern_definition() &&
            ...
            debug(1) << "Injecting realization of " << funcs[0].name() << " around node " << Stmt(for_loop) << "\n";
...
private:
    ...
    Stmt build_pipeline_group(Stmt consumer) {
        ...
        debug(1) << "producer after Letstmt:\n" << producer << "\n";
...
Stmt schedule_functions(const vector<Function> &outputs,
    ...
    debug(1) << "Stmt after initial for make in schedule_functions: \n";
    debug(1) << s << "\n";

    any_memoized = false;

    validate_fused_groups_schedule(fused_groups, env);
    debug(1) << "fused_groups num now: " << fused_groups.size() << "\n";
