...
void Pipeline::compile_to_heterocl(const string &filename,
                            const vector<Argument> &args,
                            const string &fn_name,
                            const vector<int> &output_shape,
                            const Target &target) {
    Module m = compile_to_module(args, fn_name, target);
    m.compile(Outputs().heterocl(output_name(filename, m, ".py")), contents->outputs, output_shape);
}
