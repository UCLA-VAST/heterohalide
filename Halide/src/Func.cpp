...
void Stage::set_dim_unroll_hcl(Var var, ForType t, int factor) {
    bool found = false;
    vector<Dim> &dims = definition.schedule().dims();
    for (size_t i = 0; i < dims.size(); i++) {
        if (var_name_match(dims[i].var, var.name())) {
...
Stage &Stage::unroll(VarOrRVar var, Expr factor, TailStrategy tail) {
    debug(1) << "if var.is_rvar: " << (var.is_rvar) << "\n";
    if (var.is_rvar) {
        RVar tmp;
        split(var.rvar, var.rvar, tmp, factor, tail);
        unroll(tmp);
...
Stage &Stage::unroll_hcl(Var var, int factor) {
    set_dim_unroll_hcl(var, ForType::Unrolled, factor);
    return *this;
}
...
Func &Func::unroll_hcl(Var var, int factor) {
    invalidate_cache();
    Stage(func, func.definition(), 0, args()).unroll_hcl(var, factor);
    return *this;
}
...
void Func::compile_to_heterocl(const string &filename, const vector<Argument> &args, const vector<int> &output_shape, 
                               const string &fn_name, const Target &target) {    
    pipeline().compile_to_heterocl(filename, args, fn_name, output_shape, target);
}                                   
