...
Module lower(const vector<Function> &output_funcs,
    ...
    debug(1) << "fused_groups num after realiztion_order: " << fused_groups.size() << "\n";

    debug(1) << "print order: \n"; // only a "output"
    for (auto iter = order.begin(); iter != order.end(); ++iter) {
        debug(1) << *iter << "\n";
    }
    debug(1) << "fused_groups: \n"; // only a "output"
    ...
    debug(1) << "Lowering after creating initial loop nests:\n" << s << '\n';
    
    // some optimizations to IR by Halide
    if (any_memoized) {
    ...
    s = inject_tracing(s, pipeline_name, env, outputs, t); // no change for simple
    debug(2) << "Lowering after injecting tracing:\n" << s << '\n';

    debug(1) << "Adding checks for parameters\n";
    s = add_parameter_checks(requirements, s, t); // no change for simple
    ...
    s = remove_extern_loops(s); // no change for simple
    debug(2) << "Lowering after removing extern loops:\n" << s << '\n';

    debug(1) << "Performing sliding window optimization...\n";
    s = sliding_window(s, env); // no change for simple
    debug(2) << "Lowering after sliding window:\n" << s << '\n';

    debug(1) << "Simplifying correlated differences...\n";
    s = simplify_correlated_differences(s); // no change for simple
    ...
    s = remove_undef(s); // no change for simple
    ...
    s = uniquify_variable_names(s); // no change for simple
    debug(2) << "Lowering after uniquifying variable names:\n" << s << "\n\n";

    debug(1) << "Simplifying...\n";
    s = simplify(s, false); // Storage folding needs .loop_max symbols // add a let stmt. For Simple: let input.extent.0.required.s = (min((((output.extent.0 + -1)/2)*2), (output.extent.0 + -2)) - min(output.extent.0, 2))
    debug(2) << "Lowering after first simplification:\n" << s << "\n\n";

    debug(1) << "Performing storage folding optimization...\n";
    s = storage_folding(s, env); // delete a let stmt. For simple: let input.extent.0.required.s = (min((((output.extent.0 + -1)/2)*2), (output.extent.0 + -2)) - min(output.extent.0, 2))
    ...
    debug(1) << "Simplifying...\n";
    s = simplify(s);
    s = unify_duplicate_lets(s); // reduce lots of let Stmt
    ...
    s = simplify(s);
    // s = loop_invariant_code_motion(s); // opimization: let the shared expression computed in the outer loop instead of inner loop, thus to reduce computation
