...
int main(int argc, char **argv) {
    ...
    ifstream myfile ("/curr/jiajieli/app_halide2heterocl/rand_matrix.txt");
    if (myfile.is_open())
    {     
    ...
    ofstream infile ("/curr/jiajieli/app_halide2heterocl/halide_input.txt");
    if (!infile)
    {
    ...
    //     cout << '\n';
    // }  
    cout << "input height:" << input.height() <<'\t'<< "input width:" << input.width() << endl;
    ...
    ofstream outfile ("/curr/jiajieli/app_halide2heterocl/halide_out.txt");
    if (!outfile)
    {
        cout << "can't open" << endl;
    }
    ...
    {
        for (int x = 0; x < blurry.width(); x++) {
            for (int y = 0; y < blurry.height(); y++) {
                outfile << blurry(x, y) << '\t';
            }
            outfile << '\n';
        }  
    ...
    cout << "out height:" << blurry.height() << '\t' << "out width:" << blurry.width() <<endl;
