...
public:
    GeneratorParam<int>     stencils{"stencils", 1, 1, 100}; //value, min, max. here iterations = 5

    Input<Buffer<uint16_t>> input{"input", 2}; //original dim = 2, Buffer format is uint16_t
    Output<Buffer<uint16_t>> output{"output", 2};// original dim = 2, Buffer format is uint16_t
    ...
    void generate() {
        ...
        // for (auto &s : stages){
        //     s.compile_to_lowered_stmt("stages.html", {}, Halide::StmtOutputFormat::HTML);
        // }
        }
