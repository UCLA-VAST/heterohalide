...
final_min_1 = 0
def top(input, ):
    max_local = hcl.compute((final_extent_0, final_extent_1), lambda x, y: 0, name = "max_local", dtype = hcl.UInt(bits = 8))
    with hcl.Stage("max_local"):
        with hcl.for_(final_min_1, final_extent_1, name = "max_local_s0_y") as max_local_s0_y:
            with hcl.for_(final_min_0, final_extent_0, name = "max_local_s0_x") as max_local_s0_x:
                maximum = hcl.compute((1, 1), lambda x, y: 0, name = "maximum", dtype = hcl.UInt(bits = 8))
                with hcl.Stage("maximum"):
                    maximum[max_local_s0_x, max_local_s0_y] = hcl.cast(dtype = hcl.UInt(bits = 8), expr = 0)
