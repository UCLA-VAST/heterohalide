...
def top(input, ):
    ...
    # schedule "fused" will cause weird variable like "blur_x.y.extent_realized.s" and complex expression
    # need to simplify, or create a new "fuse"?
    # blur_x = hcl.compute((hcl.select(final_extent_0 > 1, final_extent_0, 1), final_extent_1 + 2), lambda x, y: 0, name = "blur_x", dtype = hcl.UInt(bits = 16))
    blur_x = hcl.compute((final_extent_0, final_extent_1 + 2), lambda x, y: 0, name = "blur_x", dtype = hcl.UInt(bits = 16))
    with hcl.Stage("blur_x"):
        with hcl.for_(0, ((final_extent_1 + 2) * final_extent_0), name = "blur_x_s0_x_fused") as blur_x_s0_x_fused:
            t26_s = (blur_x_s0_x_fused % hcl.select(final_extent_0 > 1, final_extent_0, 1))
            t27_s = (blur_x_s0_x_fused/hcl.select(final_extent_0 > 1, final_extent_0, 1))
            blur_x[((blur_x_s0_x_fused % hcl.select(final_extent_0 > 1, final_extent_0, 1)) + final_min_0), ((blur_x_s0_x_fused/hcl.select(final_extent_0 > 1, final_extent_0, 1)) + final_min_1)] = ((input[((final_min_0 + t26_s) + 2), (final_min_1 + t27_s)] + (input[(final_min_0 + t26_s), (final_min_1 + t27_s)] + input[((final_min_0 + t26_s) + 1), (final_min_1 + t27_s)]))/hcl.cast(dtype = hcl.UInt(bits = 16), expr = 3))
