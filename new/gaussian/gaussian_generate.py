...
final_min_1 = 0
def top(input, ):
    blur_x = hcl.compute((final_extent_0, final_extent_1), lambda x, y: 0, name = "blur_x", dtype = hcl.UInt(bits = 8))
    with hcl.Stage("blur_x"):
        with hcl.for_(final_min_1, final_extent_1, name = "blur_x_s0_y") as blur_x_s0_y:
            with hcl.for_(final_min_0, final_extent_0, name = "blur_x_s0_x") as blur_x_s0_x:
                ...
                blur_x[blur_x_s0_x, blur_x_s0_y] = hcl.cast(dtype = hcl.UInt(bits = 8), expr = (sum_x[blur_x_s0_x, blur_x_s0_y]/hcl.cast(dtype = hcl.UInt(bits = 32), expr = 65536)))
    output_final = hcl.compute((final_extent_0, final_extent_1), lambda x, y: 0, name = "output_final", dtype = hcl.UInt(bits = 8))
    ...
    final = hcl.compute((392, 592), lambda x, y: 0, name = "final", dtype = hcl.UInt(bits = 8))
    ...
    return final
input = hcl.placeholder((400, 600, ), name = "input", dtype = hcl.UInt(bits = 8))
